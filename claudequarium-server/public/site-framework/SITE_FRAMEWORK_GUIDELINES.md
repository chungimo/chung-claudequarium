...
The framework lives in its own directory (e.g., `framework/`) with clear separation from site-specific code. The consuming site imports what it needs.

```
framework/
├── auth/           # JWT, bcrypt, middleware, session handling
├── db/             # SQLite setup, schema, migrations
├── ui/             # Popups, fields, buttons, notifications — reusable CSS + JS
├── settings/       # Settings panel components
├── logs/           # Log viewer component
├── README.md       # Implementation guide (see Documentation section)
└── theme.css       # Single file for all color/font/spacing tokens
```

### Theme System

All colors, fonts, border radii, spacing values, and shadows must be defined as CSS custom properties in a single `theme.css` file. Every framework component and the consuming site reference these tokens — never hardcode colors or spacing. Changing the site's visual identity should require editing only `theme.css`.

### CSS Organization

- `theme.css` — Design tokens (shared between framework and site)
- `framework.css` — Framework component styles (popups, fields, buttons, tables, settings panel)
...
Create a stub section with the framework for adding notification rules. The specific triggers will vary by project, but the structure should support:

- **Channels:** Discord, Slack, Microsoft Teams, Email.
- **Rules:** Each notification rule maps a trigger event to one or more channels with configuration (webhook URL, email address, etc.).

Leave clear documentation and placeholder UI showing where to define new trigger types and how to wire them to channels.

##### Webhooks

Create a stub section similar to notifications. Webhooks will be project-dependent. Provide the scaffolding for adding webhook endpoints, managing secrets, and configuring which events fire which webhooks.
...
- **Themeable:** One file (`theme.css`) controls the entire visual identity.
- **DRY:** One popup class, one field system, one table component. Overrides for special cases, never copies.
- **Secure by default:** Bcrypt, JWT, httpOnly cookies, role middleware, no plaintext secrets.
