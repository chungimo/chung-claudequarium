...
function login(req, res) {
  ...
  if (!user || !users.verifyPassword(user, password)) {
    logs.add('warn', `Failed login attempt for username: ${username}`);
  ...
  logs.add('info', `User logged in: ${username}`, user.id);
...
function logout(req, res) {
  if (req.user && req.user.tokenId) {
    ...
    logs.add('info', `User logged out: ${req.user.username}`, req.user.id);
  }
